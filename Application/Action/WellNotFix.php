...
class WellNotFix
{
    ...
    public function resolve(string $threadId, string $channel)
    {
        try {
            $jiraTask = $this->messenger->findJiraTaskInThread($threadId, $channel);
        } catch (NotFoundJiraTaskInThread $e) {
            $this->logger->error('Немогу передвинуть задачу в статус Wan\'t do т.к. она не была найдена в треде обращения.
             Если задача существует, закройте ее руками.');

            return;
        }

        $this->taskManager->moveToWontDo($jiraTask);
    }
}
